...
public class StrategyTwo implements IStrategy{
    ...
    private List<IVisiblePlanet> myPlanets = new LinkedList<>();
    private List<IVisiblePlanet> neutralPlanets = new LinkedList<>();
    private List<IVisiblePlanet> enemyPlanets = new LinkedList<>();
...
//    private int turn=0;
    ...
    public void takeTurn(List<IPlanet> planets, IPlanetOperations planetOperations, Queue<IEvent> eventsToExecute) {
        ...
        // Add planets to data structures
        for (IPlanet planet : planets) {
            // Add to planetMap
            int id = planet.getId();
            planetMap.put(id, planet);
            // Try to add to planet owner list
            if (planet instanceof IVisiblePlanet) {
                ...
                if (visiblePlanet.getOwner() == Owner.SELF) {
                    myPlanets.add(visiblePlanet);
                } else if (visiblePlanet.getOwner() == Owner.NEUTRAL) {
                    neutralPlanets.add(visiblePlanet);
                } else {
                    enemyPlanets.add(visiblePlanet);
                }
            }
        }
